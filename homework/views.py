...
class HomeworkAPIView(APIView):
    ...
    def get(self, request):
        ...
        if teacher:
            queryset = Homework.objects.filter(teacher=teacher).order_by("-due_date")
        elif student:
            queryset = Homework.objects.filter(teacher__batch_id=student.batch_id).order_by("-due_date")
        elif guardian:
            batch_ids = guardian.students.values_list("batch_id", flat=True)
            queryset = Homework.objects.filter(teacher__batch_id__in=batch_ids).order_by("-due_date")
        else:
    ...
    def post(self, request):
        teacher = get_teacher(request.user)
        if not teacher:
            return Response({"detail": "Only teachers can create homework."}, status=status.HTTP_403_FORBIDDEN)

        payload = request.data.copy()
        payload["teacher"] = teacher.id
        serializer = HomeworkSerializer(data=payload)
...
class HomeworkDetailAPIView(APIView):
    ...
    def get(self, request, pk):
        ...
        serializer = HomeworkSerializer(obj)
        return Response(serializer.data, status=status.HTTP_200_OK)

    def put(self, request, pk):
        teacher = get_teacher(request.user)
        if not teacher:
            return Response({"detail": "Only teachers can update homework."}, status=status.HTTP_403_FORBIDDEN)
        ...
        payload = request.data.copy()
        payload["teacher"] = teacher.id
        serializer = HomeworkSerializer(obj, data=payload)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request, pk):
        teacher = get_teacher(request.user)
        if not teacher:
            return Response({"detail": "Only teachers can delete homework."}, status=status.HTTP_403_FORBIDDEN)
